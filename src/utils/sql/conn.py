	...
	def close_DB(self, pragma:str='', attempt:int=0, max_retry:int=3, force:bool=False) -> bool:
		...
		try:
			...
			if "SQLite objects created in a thread can only be used in that same thread." in str(PE):
				self.__class__._log.warning(f"close_DB failed closing Conn '{self.key_id}' from {short(self.dbfp)}, {tb(PE)}")
			else:
				self.__class__._log.critical(f"close_DB failed closing Conn '{self.key_id}' from {short(self.dbfp)}, {tb(PE)}")
