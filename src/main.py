...
import threading
import logging, logging.handlers, sqlite3
...
def thr_sql_log(Sql_h:SqlHand, ev_quit:threading.Event, q:queue.Queue, name:str="t_sql_log"):
	...
	if not "main" in Sql_h.ConnHandler.conns:
		log.error(f"[{name}] exited unsuccessfull, no 'main' Connection in ConnectionHandler")
	...
	try:
		...
		while not last_cycle or (last_cycle and not q.empty()):
			...
			if isinstance(LRec, str) and LRec == Config.config["ExitThreadSignal"]:
				last_cycle = True
				if Sql_h.ConnHandler.conns["main"].open_status:
		...
		if Sql_h.ConnHandler.conns["main"].open_status:
			Sql_h.ConnHandler.conns["main"].close_DB()


def thr_sql_netmsg(ev_quit:threading.Event, q:queue.Queue, name:str="t_sql_netmsg"):
	global Config, log

	# create netmsgSH with Request classmethod
	netmsgSH = Request.set_netmsg(q)
	...
	# re = self.__class__.netSH._ins_one('netmsg', 
	# 							list(self.columns.values()),
	# 							self.__class__.netSH.Scheme.schemes['netmsg'].columns)

	# if not re:
	# 	self.__class__._log.error('register_Resp failed _ins_one in netmsg DB')
	...
	try:
		...
		while not quit or not q.empty():
			...
			if isinstance(msg, str) and msg == Config.config["ExitThreadSignal"]:
				quit = True
				if netmsgSH.ConnHandler.conns["main"].open_status or len(netmsgSH.ConnHandler.conns.keys()) > 1:	
					log.info(f"closing DB connection {short(netmsgSH.dbfp)}")

			# log it normally if it is LogRecord
			elif isinstance(msg, list):
				netmsgSH.ins('netmsg', msg)
		...
		#netmsgSH.ConnHandler.conns["main"].close_DB()
...
def main():
	...
	dbf = os.path.abspath(Folder.os_proj_folderpath+"/"+"test4.sqlite")
	...
	t_sql_netmsg = threading.Thread(target=thr_sql_netmsg, args=(ev_t_sql_netmsg_end, sQ_netmsg), name="t_sql_netmsg", daemon=False)
...
[1] -> select logs
[2] -> Response GET
[3] -> Response POST DATA
[9] -> {'activate' if not Request.netmsg else 'deactivate'} netmsg
[0] -> shutdown
...
Selection: ''')
		...
		if inp.lower() == "0":
			...
			re = Session.get("https://httpbin.org/get", lib="hpx")
			#re = Response("get", "https://httpbin.org/get", lib="hpx", data={})
			if re.invalid_reason is None:
				print(re.Response.content.decode())

		elif inp.lower() == "3":
			re = Session.post("https://httpbin.org/post", data={"id": 1001, "name": "geek", "passion": "coding"}, lib="hpx")
# 			re = Response("post data", "https://httpbin.org/post", lib="hpx", data={
#     "id": 1001,
#     "name": "geek",
#     "passion": "coding",
# })
